use crate::async_trait::bound::{InferredBound, Supertraits, has_bound};
use crate::async_trait::lifetime::{AddLifetimeToImplTrait, CollectLifetimes};
use crate::async_trait::parse::Item;
use crate::async_trait::receiver::{ReplaceSelf, has_self_in_block, has_self_in_sig, mut_pat};
use crate::async_trait::verbatim::VerbatimFn;
use crate::nucleus_path;
...
pub fn expand(input: &mut Item, is_local: bool, use_smallbox: bool) {
    match input {
        Item::Trait(input) => {
                ...
                if let TraitItem::Fn(method) = inner {
                    ...
                    if sig.asyncness.is_some() {
                        ...
                        if let Some(block) = block {
                            ...
                            transform_block(context, sig, block, use_smallbox);
                            method.attrs.push(lint_suppress_with_body());
                        } else {
                            method.attrs.push(lint_suppress_without_body());
                        }
                        let has_default = method.default.is_some();
                        transform_sig(context, sig, has_self, has_default, is_local, use_smallbox);
                    }
                }
            }
        }
            ...
            for inner in &mut input.items {
                match inner {
                    ImplItem::Fn(method) if method.sig.asyncness.is_some() => {
                        ...
                        transform_block(context, sig, block, use_smallbox);
                        transform_sig(context, sig, has_self, false, is_local, use_smallbox);
                        method.attrs.push(lint_suppress_with_body());
                    }
...
fn transform_sig(
    ...
    let smallbox = if use_smallbox {
        quote!(#nucleus_path::SmallBox<dyn ::core::future::Future<Output = #ret> + #bounds, Self::HandlerStorage>)
    } else {
        quote! {
            ::core::pin::Pin<Box<
...
//         ___ret
//     })
fn transform_block(context: Context, sig: &mut Signature, block: &mut Block, use_smallbox: bool) {
    ...
    let box_pin = if use_smallbox {
        quote_spanned!(sig.asyncness.unwrap().span=>
            #nucleus_path::SmallBox::<_, Self::HandlerStorage>::new(async move { #let_ret })
        )
    } else {
        quote_spanned!(sig.asyncness.unwrap().span=>
